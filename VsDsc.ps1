...
class VSComponents
{
    ...
    [bool] Test()
    {
        if(-not $this.components -and -not $this.vsConfigFile) {
            ...
            Write-Host "[OK]Config file: $($this.vsConfigFile)"
        }
        ...
        if($this.vsConfigFile) {
            if(-not (Test-Path $this.vsConfigFile)) {
                ...
                Write-Host "[OK]$($this.vsConfigFile) exists."
            }
...
function Get-VsComponents
{
    ...
    return $result.packages | Where-Object { $_.type -eq "Component" -or $_.type -eq "Workload" } | Select-Object -Property id, version, extension 
}
...
function Add-VsComponents
{
    ...
    if(-not $Components -and -not $VsConfigPath) {
        ...
        Write-Host "[OK]Componet or VSConfig is specified."
    }
    ...
    if($VsConfigPath) {
        if(-not (Test-Path $VsConfigPath)) {
            ...
            Write-Host "[OK]$VsConfigPath exists."
        }
...
function Invoke-VsInstaller
{
    ...
    $setupExe =Get-VsInstallerPath
    Write-Host "setupExe: $setupExe"
    Write-Host -foreground green "Start-Process -FilePath $setupExe -ArgumentList $Arguments -PassThru"
    $installer = Start-Process -FilePath $setupExe -ArgumentList $Arguments -PassThru
    #$installer.WaitForExit();
    $basePath = Build-BasePath
    Write-Host "basePath: $basePath"
    # Set EnableRaisingEvents to true to access the Exit Code later
    $activeInstallerProcess = Get-Process Setup
    
    if($activeInstallerProcess) {
        $activeInstallerProcess | Where-Object { $_.Path -like "$basePath*" } | ForEach-Object { $_.EnableRaisingEvents = $true }
        ...
        Wait-Process -Id $processIds -Timeout 3600      # 1 hour
        foreach($process in $activeInstallerProcess) {
            if($process.ExitCode -NotIn $validErrorCodes) {
                ...
                Write-Host "[OK]Visual Studio Installer succeeded."
            }
        }
    } else {
...
function Invoke-VsWhere
{
    ...
    Write-Host -foreground green "&'$(Get-VsWherePath)' $Arguments"
...
function Assert-IsAdministrator
{
    if(-not ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
        ...
        Write-Host "[OK]Run as Administrator."
    }
}
...
function Assert-VsInstallerPresent
{
    if(-not (Test-Path (Get-VsInstallerPath))) {
        ...
        Write-Host "[OK]Visual Studio Installer found."
    }
}
...
function Assert-VsWherePresent
{
    if(-not (Test-Path (Get-VsWherePath))) {
        ...
        Write-Host "[OK]vswhere.exe is found."
    }
}
...
function Assert-VsInstallerProcessNotRunning
{
    if(Get-Process | Where-Object { $_.Path -eq (Get-VsInstallerPath) }) {
        ...
        Write-Host "[OK]No previous Visual Studio Installer running."
    }
}
